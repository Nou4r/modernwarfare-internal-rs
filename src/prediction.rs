...
impl Target {
    ...
    pub fn from_location_history(position: &Vector3, history: &VecDeque<(Instant, Vector3)>) -> Self {
        ...
        let (vel_x, _) = linear_regression_of(&history_x).unwrap_or((0.0, 0.0));
        let (vel_y, _) = linear_regression_of(&history_y).unwrap_or((0.0, 0.0));
        let (vel_z, _) = linear_regression_of(&history_z).unwrap_or((0.0, 0.0));

        Self {
            position: position.clone(),
            velocity: Vector3 { x: vel_x as f32, y: vel_y as f32, z: vel_z as f32 },
